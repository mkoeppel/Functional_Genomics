...
5. post-processing of results from differential expression analysis
  - this includes the addition of gene-names and a variance-stabilized transformation (vst) to normalize counts and allow improved plotting
```{r}
...
```
6. graphical display of different key parameters
  - plotCounts shows the expression of the two conditions for the gene with the lowest p-value (highest significance)
```{r}

plotMA(res, ylim=c(-2,2), main="no LFC shrinkage")
plotCounts(ddsHTSeq, gene=which.min(res$padj), intgroup="condition")
plotPCA(vsd)
```
7. identification of genes with binding-sites in their vicinity
```{r}
txdb <- TxDb.Hsapiens.UCSC.hg38.knownGene
promoter <- getPromoters(TxDb=txdb, upstream=3000, downstream=3000)
mef2d_Anno<-annotatePeak(mef2d, tssRegion=c(-1000, 1000), TxDb=txdb, annoDb="org.Hs.eg.db")
